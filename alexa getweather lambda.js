        ...
        https.get(endpoint, (response) => {
          ...
          response.on('end', () => {
            ...
            if (wind_mph == 0) {
              ...
              var wind_speed_response = `wind speed is ${wind_mph} miles per hour from ${wind_dir}`
            }

            context.succeed(
              generateResponse(
                buildSpeechletResponse(`Current temperature is ${temp_f} degrees, ${wind_speed_response}`,
                  true), {}
                // buildSpeechletResponse('Current temperature is ${ temp_f } degrees, wind speed is ${ wind_mph } miles per hour `, true), {}
              ));
          });
